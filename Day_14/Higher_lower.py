...
def print_versus(compare_list):
    print(
        f"Compare A: {compare_list[0]['name']}, {compare_list[0]['description']}")
    print(vs)
    print(
        f"Against B: {compare_list[1]['name']}, {compare_list[1]['description']}")
...
def index_of_winner(compare_list):
    ...
    return compare_list.index(max(compare_list, key=lambda user: user["followers"]))
